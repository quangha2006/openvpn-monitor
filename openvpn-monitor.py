...
class OpenvpnHtmlPrinter(object):
    ...
    def print_script(self):
        ...
        for i in self.woldata:
            ...
            output('    var computername = "{0!s}";'.format(i['ComputerName']))
            output('    var id = "{0!s}";'.format(i["ID"]))
            output('    WolCommand(macaddress,computername,id);')
            output('}')
        # ping button onClick
        for i in self.woldata:
            # When the user clicks on the button, open the modal 
            ...
            output('    var id = "{0!s}";'.format(i['ID']))
            output('    PingCommand(ipaddress,id);')
            output('}')
        output('')
        output('function PingCommand(ipaddress,id) {')
        output('    xhttp.open("POST","/",true);')
        output('    var requestdata = "action=ping&&Ip-address=" + ipaddress;')
        output('    requestdata += "&&ID=" + id;')
        ...
        output('      text: "Are you sure want to wake up computer " + computername + "!",')
        output('      icon: "warning",')
        ...
        output('             const id = "text-status-" + jsonresponse["ID"];')
        output('             var text_status = document.getElementById(id);')
        output('             if (jsonresponse["ExitCode"] == 0) {')
        ...
        idArray = []
        for i in self.woldata:
            ipArray.append(i["IPAddress"])
            idArray.append(i["ID"])
        output('    let mac_array = {0!s};'.format(ipArray))
        output('    let id_array = {0!s};'.format(idArray))
        output('    for (let i = 0; i < mac_array.length; i++) {')
        output('        PingCommand(mac_array[i],id_array[i]);')
        output('        await sleep(3000);')
        output('    }') 
        output('}')
        output('')
        output('window.onload = function() {')
        output('    autocheckstatus();')
        output('}')
        output('</script>')
